...
skills: [scenario-engine, asset-spec, art-bible]
memory: project
---
...
**Quality layer:** Before writing AI generation prompts (patio tile, plant
stages, props), run `scenario-engine` mode `asset` with the art bible as canon.
Do not replace `/art-bible` or `/asset-spec` with that pass.
Production loop: `production/process/asset-pipeline.md`. Lane A = primitive + albedo.
Lane B = flat T-pose PNG, then sahip Meshy. Props are not Meshy.

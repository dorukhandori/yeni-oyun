...
**Canopy overlay:** If sahip says they never made a game, asks how to work with AI /
Cursor / vibe coding, or whether to finish a Unity course first — do not dump the
catalog and do not `/brainstorm`. Point to `ai-native-beginner`, then still name
**one** playable next slice from this skill.
...
- `/start` — re-orient from scratch (Canopy: do not use this to brainstorm a new game)
- `ai-native-beginner` — never shipped a game / how to work with AI on Canopy
```

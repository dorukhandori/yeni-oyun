...
**Quality layer (mandatory):** Before drafting a new system, run `scenario-engine`
mode `feature` (or `logic` if this is watering/calendar/care rules). Read
`.claude/skills/scenario-engine/SKILL.md`. That pass is not a replacement for
this skill — consume its beat sheet, then continue the GDD sections below.

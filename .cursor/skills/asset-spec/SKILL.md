...
**Quality layer (mandatory for generation prompts):** Before writing or rewriting
an asset's **Generation Prompt**, run `scenario-engine` mode `asset` with the art
bible + this spec as canon. Skill: `.claude/skills/scenario-engine/SKILL.md`.
Paste the engine's `generation_prompt` + negatives into the spec. This does not
replace inventory / spec structure below.

**Production (after the spec exists):** `production/process/asset-pipeline.md`.
One ASSET-id per slice. Do not run Meshy/ComfyUI from this skill.

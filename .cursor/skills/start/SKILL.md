...
- **Canopy lock (this repo):** Product is already Canopy. Never Path A/B `/brainstorm` a different game. If `design/gdd/game-concept.md` exists, skip onboarding; route to `help` (next slice) or `ai-native-beginner` (never shipped a game / AI workflow). Do not rewrite `production/stage.txt` backward to Concept.

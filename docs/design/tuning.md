...
| ~~`CYCLOPS_PHASE_OUT`~~ | ~~`58.0`~~ | s | 🔴 **Sabit sayaç modeli terk edildi (26 Ağu 2026, sahip).** *"Dev kapı açılınca dışarıya vs ışınlanmamalı, yürüyüp kapıyı açıp kendi çıkmalıdır."* Kapı durumu artık devin **gerçek konumundan** türüyor (`src/stops/cyclopsStop.ts` `GiantState` makinesi), sabit bir saniye sayacından değil. Yerini alan sabitler §12.1'in altında: `GIANT_OUT_WAIT_SECONDS` (40 s, dev dışarıda görünmezken bekliyor), giriş/dolaşma/uyku/çıkış artık devin gerçek yürüyüş hızından (`CYCLOPS_GIANT_SPEED`) ve rastgele hedef sayısından **türeyen** bir süre — sabit değil. |
| ~~`CYCLOPS_PHASE_RETURN`~~ | ~~`8.0`~~ | s | 🔴 **Terk edildi, yukarıya bkz.** Kavram olarak hâlâ var (dev eşiğe yürürken kapı hâlâ açık, toplama mümkün) ama süresi artık dev-eşik mesafesine (giriş noktası `z=-3`) ve `CYCLOPS_GIANT_SPEED`'e bağlı — ölçülen: **~1 s** (çok kısa, giriş noktası eşiğe yakın seçildi). `GIANT_ENTER_START_Z`'nin `level-cyclops-cave.md` §4.8'in **2.7 s tepki payı** gerekçesiyle yeniden değerlendirilmesi gerekebilir — henüz yapılmadı, 🔬. |
| ~~`CYCLOPS_PHASE_PRESENT`~~ | ~~`30.0`~~ | s | 🔴 **Terk edildi, yukarıya bkz.** Artık devin dolaşma (2 rastgele nokta girişte, 1 çıkışta) + uyku (`GIANT_SLEEP_SECONDS`=8 s, 🔬 primitif başlangıç) + gerçek yürüyüş süresinin toplamı — **ölçülen aralık ~32–47 s** (hedefin derinliğine göre değişiyor, bkz. commit `<K5-K11 sonrası dev state machine commit'i>`). Eski "en derin gezinme = 30 s'nin tam karşılığı" gerekçesi artık geçerli değil, çünkü tek yönlü yürüyüş değil çok-noktalı dolaşım var. |
| ~~`CYCLOPS_CYCLE`~~ | ~~`96.0`~~ | s | 🔴 **`[TÜRETİLMİŞ]` sabiti geçersiz — artık türetilecek tek bir sayı yok.** Ölçülen tam döngü (dışarıda bekleme + giriş + dolaşma + uyku + dolaşma + çıkış): **~78.5 s** (sığ hedef, en kısa) **ile ~154 s** (iç nöy gibi derin hedef + uzun dolaşma, en uzun) arasında değişiyor — 3 tam döngü ölçümünde ortalama ~80 s. §11.7'nin "ilk-geçiş medyan döngü sayısı <3" kriteri bu yeni, değişken döngü süresiyle **yeniden ölçülmeli**, eski sabit 96 s varsayımıyla hesaplanamaz. |
...
| **`CYCLOPS_CRUSH_CAP`** *(yeni)* | `3` | adet | **🟡 Kayıp koşulu — sahip kararı, 25 Ağu 2026, DENEYSEL (kesin değil).** ⚠️ **25 Ağu, sahibin kendi eleştirisi üzerine düşürüldü:** bu sayı hiç oynanmadan, devin rotası kasıtlı öngörülemez tutulurken beş dakikada karara bağlandı — `@helix`'in gerekçeli "kayıp koşulu yok" önerisinin yerine geçti ama kendisi hiç test edilmedi. **İmplementasyon** (`cyclopsStop.ts`, K1-K11 sonrası iki turda değişti — sayı hâlâ tek satırda değiştirilebilir bir sabit, "kesin" muamelesi görmüyor): 1. ve 2. yakalanmada yalnız D2/C2 cezası işler (azık zemine düşer, `DETECT` sıfırlanır, 2 sn bağışıklık) — **oyuncu artık hiç ışınlanmıyor**, tam yakalandığı yerde kalıyor (sahip, 26 Ağu üçüncü tur: "dev beni bir kere ezdiğinde girişe ışınlanıyorum, bunu ben sadece 3/3 olduğunda istiyorum"). 3. yakalanmada durak **başarısız** olur — ama artık sessiz bir hub-dönüşü değil, **tam ekran "KAYBETTİN" durağı** (sahip, 26 Ağu ikinci tur): simülasyon donuyor, "Yeniden Oyna" tıklanana kadar hiçbir şey ilerlemiyor; tıklanınca **o denemedeki tüm ilerleme sıfırlanır — teslim edilmiş azık dahil** (`delivered` → 0, azık yerleşimi başa, gemi başlangıcına ışınlanma). **Kalıcı ceza yok:** durak sınırsız kez yeniden denenebilir. **Ekranda gösterilmez** (P2) — kalan hak, her ezilmede ağırlaşan korku efektiyle hissettirilir. D3'ün (unutuş yok) açtığı "hiç kayıp koşulu kalmadı" boşluğunu kapatır. Ayrıntı: `gdd-cyclops-blinding.md` bitiş/kayıp sözleşmesi, §11.7'nin ölçüm maddesi. |
| ~~`CAUGHT_RESPAWN_POINT`~~ | 🔴 **YOK** | — | **Kaldırıldı (26 Ağu 2026, sahip, üçüncü tur).** ~~`"cave_mouth"` (D≈4)~~ — bir ara turda D≈4'ün devin kendi dolaşma/beeline hattının üstünde olduğu bulunup `x=2.4,z=−4`'e (path koridoru) taşınmıştı, ama hemen ardından mekanik tamamen kaldırıldı: 1./2. yakalanmada **artık hiç ışınlanma yok**, oyuncu tam yakalandığı yerde kalıyor. Işınlanma yalnız 3/3'te ve yalnız "Yeniden Oyna" tıklanınca (gemi başlangıcı `-18`'e), bkz. `CYCLOPS_CRUSH_CAP` satırı. |
| `CYCLOPS_ITEM_TOTAL` 🔬 | ~~`7.0`~~ → **`6`** (26 Ağu, sahip) | adet | Dağılım iki kez değişti: önceden 2 depo/3 ağıllar/2 iç nöy → 2/2/3 (26 Ağu, "en çok dev'in odasında yemek olacak") → **2 depo/1 ağıllar/3 iç nöy** (26 Ağu, ikinci tur, A-02 kaldırıldı). Sahip: "yerdeki yemekler gemiye yetmiyor, dev'in odasına girmeleri gerekiyor" — bu, aşağıdaki §9 madde 5'in açık sorusunu **kasıtlı olarak kapatıyor**: depo+ağıllar artık yalnız `3` < hedef `4`, İç nöy'e girmeden durak bitirilemez. Karşılığında İç nöy 3 gizlenme girintisi aldı (diğer odalar 1) ve rage/telgraf saldırısı odanın her köşesini kapsıyor (dev'e yakınlık değil oyuncunun konumu hedefleniyor) — "güvenli köşe" artık yok. |
...
| `CAUGHT_ITEM_DROP`, `CAUGHT_DROP_RADIUS`, ~~`CAUGHT_RESPAWN_POINT`~~ (26 Ağu, kaldırıldı) | 7→**6** azığın **koordinatları** ve tipleri; düşen öğenin zemine snap kuralı |
| `CYCLOPS_ITEM_TOTAL` (~~7~~→**6**), `CYCLOPS_ISLAND_TARGET` (4), `CYCLOPS_LIGHT_RADIUS*` | `CYCLOPS_CAVE_DEPTH` (65 m), `CYCLOPS_COVE_DEPTH` (15 m), oda-başı öğe sayıları (~~2/3/2~~ → 2/2/3 → **2/1/3**, 26 Ağu) |

### 12.3 Dash / sürünme / dev rage — YENİ (26 Ağu 2026, sahip, henüz `gdd-cyclops-blinding.md`'ye işlenmedi)

> Kod önce, doküman notu burada — bu üç mekanik `cyclopsStop.ts`'te 🟡 deneysel sabitler olarak yaşıyor, henüz `gdd-cyclops-blinding.md`'nin kendi otoritesine taşınmadı. Sahip: "dash movement da olacak, yerde sürünme gibi" + "dev bazen rage geçirecek kendi odasında, sarhoş gibi hareket edecek, aRPG bosslarındaki gibi önceden yuvarlaklarla gösterilmeli."

| İsim | Değer | Gerekçe |
|---|---|---|
| `DASH_DISTANCE` / `DASH_DURATION` / `DASH_COOLDOWN` | `4.5 m` / `0.15 s` / `1.4 s` | Shift (veya Q — Shift'in bazı ortamlarda `event.code` boş gelmesi bulunup ikinci bağımsız tuş eklendi, 26 Ağu). Tek atılımlık patlama, dodge amaçlı (asıl kullanım yeri: rage telgraflarından kaçmak). |
| `CRAWL_SPEED_MULT` / `CRAWL_DETECT_MULT` | `0.45` / `0.4` | Ctrl veya C, tutarken. Hız cezası + `DETECT` birikim indirimi (asıl gizlenme faydası). |
| `CYCLOPS_RAGE_CHANCE` / `_DURATION` / `_SPEED_MULT` | `0.4` / `7.0 s` / `1.7×` | Yalnız İç nöy'de, yalnız uyanıkken (`wanderingPre`/`Post` varışında çekiliş) — `sleeping`'e karışmıyor, "uyuyan dev izlemiyor" kuralı korunuyor. |
| `CYCLOPS_ATTACK_TELEGRAPH_SECONDS` / `_INTERVAL` / `_RADIUS` | `1.1 s` / `2.3 s` / `2.4 m` | Kırmızı halka decal, dodge penceresi. Hedef **oyuncunun gerçek konumu**, yalnız İç nöy'ün kendi sınırlarına kırpılı (`CYCLOPS_ATTACK_ROOM_MARGIN_*`) — sahip (ikinci tur): "güvenli alanları da kapsasın, her yere vurabilsin" — hiçbir köşe/gizlenme girintisi artık kalıcı güvenli değil, kaçınma yalnız zamanlı hareketle mümkün. |

Doğrulama: hepsi `__CYCLOPS_DEBUG__` ile (dash mesafesi, crawl hız/DETECT oranı, rage tetiklenme sıklığı, telgraf isabet/kaçınma) — commit mesajları `git log`'da, ayrıntı burada tekrarlanmadı.

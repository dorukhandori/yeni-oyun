...
| K12 | ✅ **Yapıldı (primitif stil, gerçek disiplin), 25 Ağu.** Hedef sayacı (teslim/taşınan) + fazla belirginleşen "Saklan!" uyarısı (sayaç yok, P2) + prompt mesaj satırı. `src/ui/hud.ts` **kullanılmadı** — Cyclops kendi izole DOM'unu kuruyor (aynı A3 gerekçesi: Lotus'a dokunmadan). DETECT/ezilme sayısı **hiç gösterilmiyor** (bilinçli, tuning.md §12). Yan bulgu: `#hud` (Lotus'un statik paneli) varsayılan görünürdü, gizleme listesine eklendi | `src/stops/cyclopsStop.ts` | ~0,7 oturum | K9 |
| K13 | 🟡 **Kısmen — sahip playtest'i sonrası, 25 Ağu.** Hub kartı artık tıklanınca `?stop=cyclops`'a yönlendiriyor (tam sayfa navigasyon), oradan direkt oynanabilir mekaniğe düşüyor — sahip normal yoldan (Title→Oyna→Hub→kart) hiç ulaşamıyordu, önce buna acil ihtiyaç vardı. **Eksik:** gerçek kilit kontrolü yok (Lotus bitmeden de tıklanabiliyor, bilerek — test kolaylığı), kilit kalıcılığı (`localStorage`) yok, kart görsel "kilitli" durumu hâlâ eski | `src/ui/menu.ts` | ~0,3 oturum (bu kısım) | K1 |

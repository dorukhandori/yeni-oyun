...
- **Asset bütçesi aşımda (ölçüldü, 16 Ağu 2026, `npm run test:assets`):** `public/assets/` **10.12 MB**, `pipeline.md` §6 hedefi **8 MB** (%26 aşım; 23 dosya hâlâ PNG, toplam 9.01 MB). Ayrıca `ref/` altındaki 1.90 MB oyuna girmiyor ama `public/` altında olduğu için kullanıcıya iniyor. Karar: **K37**.
- **Manifest bütünlüğü kırık (aynı ölçüm):** 4 hata — `ui/title_bg_key_art_1344.webp` Title/Hub'da canlı kullanılıyor ama `assets.csv` satırı yok; ASSET-021/030/037'nin dosyaları diskte yok. Ayrıntı: `docs/production/asset-testing-platform.md` §5. Bu satır §42'deki "registry ve assets.csv güncel ve tutarlı" ifadesini **kısmen yalanlıyor** — tutarlılık ölçülene kadar varsayımdı.
...
**Tahmini kapsam:** 6–8 oturum (Lotus Adası'nın çekirdek mekaniği, 2.1–2.9) **+ 🔴 ~18–19 oturum (24 Ağu 2026, D8 — Kiklop Mağarası, 2.6b/2.6d/2.6e)** **+ tahmini 1–2 ek oturum** (14 Ağu 2026, sanrı figürleri — 2.10) **+ Sirenler (2.6c) henüz tahmin edilmedi** (level-spec yazılmadı). **Toplam ~26–30 oturum**, Sirenler hariç.

> ### 🔴 D8 — Kiklop tahmini üç kez büyüdü (24 Ağu 2026)
>
> | Tahmin | Dayanağı | Sayı |
> |---|---|---|
> | ~~Bu satırın eski hâli~~ | 14 Ağu — hub'sız, `Stop` mimarisiz, asset'siz | ~~**3**~~ |
> | `cyclops-cave-production-plan.md` 1. tur | 24 Ağu sabahı — hub + A3 + P-A silüet + tek yeni sistem | ~~**10**~~ |
> | **Geçerli** | 24 Ağu — sahip kararlarından sonra | **~18,5** |
>
> **Farkı yaratan dört kalem (hepsi sahip kararı):** zorunlu Gemini konsept-onay kapısı **+5** (D4-ek) · Polyphemos P-A→**P-C** **+2** (D4, ~80 kredi) · **körleşme** ikinci yeni sistem **+1,5** (D3) · korku temasının **ses katmanı** **+1**. Düşen: unutuş taşıma **−0,5** (K40).
>
> ⚠️ Onay turları öngörülemez — oda konseptleri ortalama **iki** tur alırsa **~22–23**. Kırılım, risk ve kapsam küçültme seçenekleri: `docs/production/cyclops-cave-production-plan.md` **§6.1**. Kapsam eskiye göre kabaca 2.6 özelinde **3× büyüdü** (1 durak → 3 durak), Kiklop'a özgü yeni bir sistem (algılanma) ve şimdi Lotus Adası'na özgü ikinci yeni bir sistem (sanrı figürleri) eklendi; 2.1–2.5 ve 2.7–2.9 paylaşılan sistemler olduğu için bir kez yazılıyor, çarpılmıyor.
...
| ~~2.6d~~ | 🔴 **DÜŞTÜ (K40, 24 Ağu 2026).** ~~Hub'a dönüş: `MEM_ISLAND_RELIEF_PCT` formülünün uygulanması + hub'dan durak seçimi akışı~~ — duraklar arası hiçbir durum taşınmadığı için `MEM_ISLAND_RELIEF_PCT` uygulaması **gereksiz**, iş kalemi kapatıldı. **Yerine geçen (küçük) iş:** Lotus bir kez bitince Kiklop kilidinin **kalıcı** açılması — tek `localStorage` boolean'ı, `menu.ts`'in mevcut oturum-içi `setCyclopsReady()`'sinin kalıcılaştırılması. Bkz. `multi-island-concept.md` §10.5. **Tahmin: 0,25 oturum** (eski 2.6d'nin yerine). | `gameplay-programmer` |
| 2.6e *(14 Ağu 2026; **24 Ağu 2026'da kapsamı büyüdü**)* | Kiklop Mağarası'nın **iki katmanlı tehdit sistemi**: (a) **körleşme** — kapı açık/kapalı döngüsü, kapalıyken toplama devre dışı, oda-başı saklaş noktaları, "hareketsiz kal" kuralı, devin rastgele derinliğe gidip uyuması, ezilme = yakalanma (`gdd-cyclops-blinding.md`, **yeni sistem, 24 Ağu**); (b) **algılanma** — `DETECT_*` 2×2 oran matrisi + evre çarpanları, aynı zaman çizgisinde (`gdd-detection-cyclops.md`). Yakalanma: çantadaki azık yere düşer (**yok olmaz**, D2/C2), oyuncu mağara ağzına ışınlanır. **`CAUGHT_MEM_SPIKE` kaldırıldı — bu adada unutuş sistemi yok** (D3). Sabitler `tuning.md` §12. 2.6b'ye bağımlı. | `gameplay-programmer` |
...
| **K40** *(24 Ağu 2026)* | **Duraklar bağımsız mı, tek koşunun parçası mı?** M7/K27–K30 tüm yapıyı "3 duraklı tek koşu" (unutuş taşınır, `RUN_TARGET_TOTAL = 12` paylaştırılır) olarak kurmuştu. | **kapandı (24 Ağu 2026, sahip):** *"Zaten Lotus adasını bitirmeden Kiklop açık olmayacak. Bir sefer de bitirdiğinde artık Kiklop ve Lotus adası oynanabilir olacak her zaman."* → **Duraklar bağımsız.** Lotus'u bir kez bitirmek Kiklop'u **kalıcı** açar; sonrasında her durak hub'dan bağımsız seçilen, kendi başına biten bir oturum. Duraklar arası **hiçbir durum taşınmaz**. **Düşenler:** `MEM_ISLAND_RELIEF_PCT` (+ Faz **2.6d** iş kalemi), `RUN_TARGET_TOTAL` ve alt-hedef paylaştırması, `tuning.md` §11.4 ölçümü + ölçüm listesi madde 7, K27/K28'in koşu-varsayımlı kısımları, K30 madde 1/2/3/5. **Geçerli kalan:** 3 durak kimliği (K29), hub'ın kendisi, her durağın kendi hedefi. **Yeni küçük iş:** kilit kalıcılığı (tek `localStorage` boolean'ı). Ayrıntı: `multi-island-concept.md` §10. | (kapandı) |
| ~~**K27**~~ **— konusuz kaldı (K40, 24 Ağu 2026):** duraklar-üstü bir koşu olmadığı için "koşu-bazlı kayıp" diye bir seçenek yok. Her durak kendi kayıp sözleşmesini kendi tanımlar. Aşağısı arşiv. | **Koşu-bazlı kayıp onayı** (`multi-island-concept.md` M4'ün örtük sonucu, ayrıca soruldu) — bir durakta unutuş dolarsa **tüm koşu** mu biter, yoksa ada bazlı bir checkpoint mi var? | **kesin kapandı (14 Ağu 2026, ikinci ve son karar):** hub geri geldiği için ilk karar (tüm koşu biter) geçersiz kaldı, yeniden açıldı (bkz. K30 madde 4), ve şu şekilde kapandı: **sadece o durak biter** — oyuncu hub'a döner (bağışlamasız, `MEM_ISLAND_RELIEF_PCT` uygulanmaz), başka bir durak ya da aynısını tekrar seçebilir. `gdd-memory-system.md` §3.1 madde 9 buna göre güncellenecek. | (kapandı) |
| ~~**K28**~~ **— revize (K40, 24 Ağu 2026):** "~20–30 dk, koşunun tamamı" düştü; geçerli hedef **durak başına ~5–10 dk**, duraklar-üstü bütçe yok. Hub'ın hafif kalması kısıtı korundu. Aşağısı arşiv. | **Oturum süresi genişlemesi** — `game-concept.md` §2'deki "5–10 dakika" koşunun tamamı için mi, tek durak için mi geçerli olacak? | **kapandı (14 Ağu 2026):** koşunun tamamı için **~20–30 dakika**. Her durak eski tek-ada ölçeğinde (~5–10 dk) kalıyor. `game-concept.md` §2'ye yazıldı. **Ek not (aynı gün, hub'a dönüş sonrası):** sayı **değişmedi** — hub bir seçim ekranı olarak tasarlanmalı (gezinme süresi ihmal edilebilir), yoksa bütçe şişer. `multi-island-concept.md` §9.4. | (kapandı) |
...
| **K30** *(14 Ağu 2026)* — ⚠️ **madde 1/2/3/5 K40 ile geçersiz (24 Ağu 2026):** unutuş taşıma düştü, `RUN_TARGET_TOTAL` düştü, oturum süresi durak-başına indi, hibrit kilit yerine **kalıcı kilit** geldi. Madde 4 (durak kaybı → hub) geçerli. | **Hub geri geldi — `multi-island-concept.md` M7'nin "hub yok" kararını tersine çeviriyor.** Sahip artık gerçek bir Hub (Başlık → Ada seçimi) istiyor. Bu, K27 (koşu-bazlı kayıp) ve M4'ün (unutuş taşıma formülü) hub'sız-koşu varsayımıyla nasıl bir arada duracağını yeniden açıyor. | **tamamen kapandı (14 Ağu 2026):** (1) **Unutuş taşıma (M4):** hub sıfırlamıyor, `MEM_ISLAND_RELIEF_PCT` tetiği "hub'a dönüş"e taşındı, yalnızca başarılı tamamlanışta uygulanıyor. (2) **`RUN_TARGET_TOTAL`:** değişmedi, hâlâ 12/durak-başı-sabit-alt-hedef. (3) **Oturum süresi:** ~20–30 dk değişmedi. (4) **Durak kaybı:** sahip onayladı — **sadece o durak biter**, oyuncu hub'a döner (bağışlamasız). Bkz. K27. (5) **Hub kilit modeli:** sahip **Seçenek C — Hibrit** onayladı (ilk koşuda sıralı kilit Lotus→Kiklop→Sirenler, koşu bir kez bitince o oturumda serbest seçim). `ux/screens.md` §3.3'e işlenecek. | (kapandı) |

| **K37** *(16 Ağu 2026)* | **8 MB ilk-indirme hedefi hâlâ geçerli mi?** Bugün ölçüldü: **10.12 MB** (%26 aşım; 23 dosya hâlâ PNG, 9.01 MB). `pipeline.md` §6 bu sayıyı zaten `[P][?]` (ölçüm sonrası düzeltilecek) işaretlemiş. Ya hedef yükselir ya WebP borcu kapanır — ikisi de meşru, ama biri seçilmeli. Ölçüm: `docs/production/asset-testing-platform.md` §5. | açık | Asset kabul kapısının kalıcılaşması: karar verilmeden `npm run test:assets` **hiçbir zaman yeşile dönmez**, dönmeyen kapı koşulmaz. |
| **K38** *(16 Ağu 2026)* | **Image-to-3D hattı.** Sahip ön plan billboard ağaçları kâğıt reddetti; hacimli mesh istiyor. Higgsfield/Gemini mesh vermez. | İlk iskelet Meshy idi. **K39 ile satıcı Tripo V3.** | K39 |
| **K39** *(16 Ağu 2026)* | **Image-to-3D satıcısı = Tripo OpenAPI v3** (Meshy değil). Studio ile API ayrı cüzdan. | **script bağlandı, üretim yok.** `scripts/gen-mesh.mjs`: `POST /v3/files` → `image-to-model` (`v3.1-20260211`, `texture/pbr false`, `smart_low_poly`, `auto_size`) → poll 2 sn → GLB (`model_url` ~5 dk). `TRIPO_API_KEY` `.env.local` (git dışı). İlk GLB G1: tek prop (kaya), koruluk değil. | ilk G1 |
...
| **K35** *(15 Ağu 2026)* | **Lotus kenar görev (Beş yeter).** | **Kodlandı.** Lotus kartı asıl 12’li oyunu tutar; **Beş yeter** = 5 rastgele, gün döner, forget event. | Kiklop level’ı hâlâ yok (rozet açılır) |
| **K36** *(16 Ağu 2026)* | **Araştırma turu çıktıları — hangileri alınıyor?** 16 Ağu 2026'da dört doküman üretildi; indeks: `docs/research/lotophagoi-research-index.md`. Alt kalemler: **(a)** `npm run test:assets` kalıcı kabul kapısı mı (`pipeline.md` §8'e satır olarak eklendi — geri alınabilir), **(b)** görsel öncelik sırası V9→V7→V2→V10 onaylanıyor mu (`lotophagoi-visual-quality-benchmark.md` §6), **(c)** agent roster güncellemesi uygulanacak mı (`docs/production/agent-roster-update-proposal.md` — sahip "uygulansın" dedi ama `.claude/agents/**` ve `CLAUDE.md` düzenlemeleri izin sistemi tarafından bloklandı, **sahibin kendi eliyle yapması gerekiyor**), **(d)** `scripts/pipeline/` taşıması (üretim ≠ doğrulama, `asset-testing-platform.md` §3), **(e)** 4 yeni devDependency (~6.7 MB + repo dışı ~150 MB Chromium cache) kurulsun mu. | açık |

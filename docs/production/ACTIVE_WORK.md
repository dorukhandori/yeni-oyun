...
| Claude (ana oturum, `@glyph`'in yarım kalan işini devraldı) | 2026-08-17 | `workbench.html`, `src/workbench/{main.ts, viewer.ts, workbench.css}`, `vite.config.ts` (yeni middleware) | LOT-32 **tamamlandı, gerçek genişlikte (1280×800) tarayıcıda doğrulandı** (Paca'da Done). Sırasıyla bulunup düzeltilen iki hata: (1) `[hidden]` CSS'te eziliyordu (sekmeler üst üste), (2) `<canvas>`'ın flexbox'ta replaced-element min-width'i paneli ekran dışına itiyordu (sahip kendi tarayıcısında "bomboş sahne" olarak yakaladı) — `min-width:0` ile düzeltildi. Sahip isteğiyle iki özellik eklendi: "var olan assetler" dropdown'u (`/__workbench/models` dev-only endpoint) ve "dış klip ekle" (ayrı bir dosyanın kliplerini yüklü modele bağlama — LOT-37'nin ihtiyacı). `tsc`+`build` temiz, `dist/` hâlâ temiz. Sahip onayı + commit bekliyor. |

...
| Claude (`producer` / `@nile`, bu oturum) | 2026-08-17 | `src/render/{stage,sunDisk,hazePass}.ts`, `src/constants.ts` (**yalnız** `RENDER` / `DAY` / `SUN_DISK` / `SKY_TEX` blokları) | Gökyüzü toparlama turu (LOT-60): iki gökyüzü küresi arası dikiş/blend, sis + ufuk mesafeleri `ISLAND_RADIUS` 160 m sonrası, güneş diski/sungod tutarlılığı (`dayProgress` 0/0.5/1), `test` profili, palet uyumu, haze maliyeti. `@iris` görsel yargı + `@byte`/`@axiom` shader tarafı. **`world/` altına dokunmuyorum** — ada donatım (LOT-53) ve deniz/gemi dilimiyle dosya çakışması yok. Günü kapatan iş. |

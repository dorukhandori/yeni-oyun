...
3. **Bağlantı nereye?** ✅ **Claude Code tarafı tamamen bağlı ve doğrulandı (2026-08-17).** `claude mcp add` bu ortamda çalışmadı (bu masaüstü uygulaması standalone CLI değil, kendi gömülü `claude` ikilisini kullanıyor) — gerçek mekanizma `~/Library/Application Support/Claude/claude_desktop_config.json`'a `mcpServers.blender` girişi eklemekmiş (Claude Desktop'ın "Settings → Developer → Edit Config" akışının dosya karşılığı), ana oturum ekledi, sahip restart etti. Eklenti dosyası kopyalandı (`uvx blender-mcp install-addon`), sahip Blender'da Preferences → Add-ons'tan etkinleştirip "Start MCP Server"a tıkladı. `mcp__blender__get_addon_status` + `get_scene_info` ile canlı doğrulandı: Blender 5.2.0 LTS, eklenti [1,5], protokol 4 (uyumlu), gerçek sahne (`Cube`/`Light`/`Camera`) okundu. **LOT-33 Done.** Cursor tarafı hâlâ ayrı, henüz yapılmadı — Settings → MCP'den aynı `{"command":"uvx","args":["blender-mcp"]}` girişi sahip tarafından eklenmeli, isterse.
...
|---|---|---|---|
| 0 | ✅ §4 workbench'in ilk sürümü — §4.5'teki `vrm-viewer` deseni (framework yok, sekmeli panel), `workbench.html`. Test matrisi (2026-08-17): Model sekmesi klipsiz GLB ile, Animasyon sekmesi 3 klipli GLB ile — ikisi de doğrulandı, `commit ab05d22` ile pushlandı | `ui-programmer` (LOT-32, Paca — **Done**) | yok |
| 1 | ✅ Blender kuruldu + `ahujasid/blender-mcp` bağlandı + canlı doğrulandı (`get_addon_status`/`get_scene_info`, 2026-08-17) | sahip + ana oturum (LOT-33, Paca — **Done**) | yok |
| 2 | Blender MCP ile **tek bir basit prop** dene (§2 tablosundaki G1 mantığı — koruluk değil, tek kaya) — canlı döngü olmadan, tek atım, sadece bağlantı çalışıyor mu diye | `technical-director` ya da `gameplay-programmer` (LOT-34, Paca) | 1 — **artık açık, sıradaki iş** |

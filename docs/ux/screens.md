...
> **🔴 K40 (24 Ağu 2026, sahip) — kilit modeli değişti, §3.3'ün C (Hibrit) kararı geçersiz.** Yeni kural: **Lotus'u bir kez bitirmek Kiklop'u kalıcı olarak açar**; sonrasında ikisi de her zaman seçilebilir. Bu, C'nin "koşu bir kez bitince **o oturumda** serbest" davranışından farklı — kilit **oturumlar arası kalıcı** (`localStorage`). Ayrıca **duraklar-üstü "koşu" kavramı kalktı**: §10'un durak-bazlı/koşu-bazlı kayıp ikilemi (S3) **konusuz kaldı**, "Koşu sonu" ekranı (`ia.md` U6/M4, `user-flow.md` §10) da karşılıksız — her durak kendi Ayrılış'ıyla biter ve hub'a döner. §3.3'ün A/B/C tablosu **arşiv**. Gerekçe: `docs/design/multi-island-concept.md` §10.
> **Durum:** tasarlandı — **Hub eklendi (2026-08-14)**, ~~kilit/ilerleme mekanizması kapandı: sahip Seçenek C (Hibrit) onayladı (bkz. §3.3)~~ **→ K40 ile değişti, yukarı bak.** Durak kaybı: **sadece o durak biter**, oyuncu hub'a döner (hâlâ geçerli; ~~K27~~ konusuz kaldı).
...
- Sağ-üst **sesi kapat** ikonu yalnızca Title ve Hub'da durur (`src/ui/mute.ts`). Mute *durumu* oyun içinde de geçerlidir — kontrol kaybolur, sessizlik kaybolmaz. Tam ekran jest zincirine bağlanmaz.
...
Uygulama: `src/ui/fullscreen.ts`, `src/ui/mute.ts`. Portrait çevirme kapısı ayrı durur (`src/ui/orientation.ts`).
